...
## 背景知识回顾：函数栈帧 (Background Recap: The Function Stack Frame)
...
*   **返回地址**：函数执行完毕后应该返回到哪里。
*   **保存的寄存器**：尤其是调用者的`ebp`值，以确保函数返回后，调用者的栈帧能被正确恢复。

### 关键寄存器与函数序言

栈帧的管理依赖于两个关键的CPU寄存器：

*   **`esp` (Stack Pointer)**：栈顶指针。它总是指向栈的当前顶部，其值会随着数据的压栈（`push`）和出栈（`pop`）而改变。
*   **`ebp` (Base Pointer)**：基址指针。与`esp`不同，`ebp`在函数的大部分执行时间内保持一个固定的值。它像一个“锚点”，为访问栈帧内的局部变量和参数提供了一个稳定的参考基准。

为了建立这个“锚点”，几乎每个函数在执行之初都会运行一段被称为**函数序言 (Function Prologue)** 的固定代码：

1.  `push ebp`：将调用者函数的`ebp`值压入栈中进行保存。
2.  `mov ebp, esp`：将当前的`esp`值赋给`ebp`，从而为当前函数确立新的栈帧基址。
3.  `sub esp, N`：将`esp`向下移动`N`个字节，为所有局部变量“预留”出空间。

当函数结束时，相应的**函数结语 (Function Epilogue)**（通常是`leave`和`ret`指令）会执行相反的操作，恢复调用者的`ebp`和`esp`，拆除当前栈帧，并返回到调用点。

### 与本挑战的联系

**这套机制正是本挑战的核心所在。** 我们的angr脚本选择从`handle_user`函数的**中间**开始执行，完全跳过了上述的函数序言。因此，angr的模拟环境中并没有一个正确设置好的栈帧。

这就是为什么我们的解题脚本必须手动模拟这些步骤：
*   我们用 `initial_state.regs.ebp = initial_state.regs.esp` 来模拟 `mov ebp, esp`。
*   我们通过 `[ebp - 偏移量]` 的方式来计算地址，因为我们知道局部变量就是这样相对于`ebp`定位的。
*   我们用 `state.memory.store()` 直接将符号化的密码写入这些地址，完美地重现了`scanf`本应完成的工作。

理解了函数序言，您就能明白为什么我们的angr脚本需要进行这些看似复杂的“手动设置”。
...
2.  **编译选项**：`generate.py` 使用 `gcc -fno-stack-protector` 进行编译。这个选项会禁用“栈金丝雀”（stack canaries）机制。栈金丝雀是一种在函数栈帧中插入随机值的安全技术，程序在函数返回前会检查该值是否被篡改，以检测并阻止栈缓冲区溢出攻击。禁用金丝雀后，栈上不会再有随机且不可预测的检查值，使得栈布局变得稳定可预测，因此我们能够准确定位和覆盖返回地址，这是本挑战能够进行的关键。
...
4.  **成功条件**：
    ...
    *   只有当 `complex_function0` 的返回值等于 `USERDEF0` **并且** `complex_function1` 的返回值等于 `USERDEF1` 时，程序才会打印 "Good Job."。
    ```c
    // The condition (a ^ b) is equivalent to (a != b) for integers.
...
**这对我们意味着什么？**
我们不能再依赖 angr 自动处理 `scanf`。相反，我们需要完全接管程序的初始状态，跳过 `scanf` 调用，并手动在栈内存中创建两个符号变量，精确地放置在 `user_int0` 和 `user_int1` 应该在的位置。
