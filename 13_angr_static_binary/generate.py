...
def generate(argv):
  ...
  with tempfile.NamedTemporaryFile(delete=False, suffix='.c', mode='w') as temp:
    ...
    if arch.startswith("x86"):
      # 适用于 x86_64 架构的编译命令
      compile_cmd = 'gcc -static -fno-pie -no-pie -fcf-protection=none -fno-stack-protector -m32 -O0 -g -o ' + output_file + ' ' + temp.name
    elif arch == 'arm64':
      # 适用于 arm64 架构的编译命令 (Apple Silicon)
      compile_cmd = 'gcc -static -fno-stack-protector -O0 -g -o ' + output_file + '_arm' + ' ' + temp.name
    else:
      # 其他架构的默认编译命令
      compile_cmd = 'gcc -static -fno-stack-protector -O0 -g -o ' + output_file + '_other' + ' ' + temp.name
