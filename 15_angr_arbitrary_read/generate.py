...
#  rodata_parts = ''.join([ chr(random.randint(ord('A'), ord('Z'))) for _ in xrange(3) ] + [ chr(random.randint(0,16) + rodata_tail_modifier) ])
  ...
  with tempfile.NamedTemporaryFile(delete=False, suffix='.c', mode='w') as temp:
    ...
    if arch.startswith("x86"):
      # 适用于 x86_64 架构的编译命令
      compile_cmd = 'gcc -fno-pie -no-pie -fcf-protection=none -fno-stack-protector -m32 -O0 -g -Wl,--section-start=.rodata=' + rodata_address + ' -o ' + output_file + ' ' + temp.name
    elif arch == 'arm64':
      # 适用于 arm64 架构的编译命令 (Apple Silicon)
      compile_cmd = 'gcc -fno-stack-protector -O0 -g -Wl,--section-start=.rodata=' + rodata_address + ' -o ' + output_file + '_arm' + ' ' + temp.name
    else:
      # 其他架构的默认编译命令
      compile_cmd = 'gcc -fno-stack-protector -O0 -g -Wl,--section-start=.rodata=' + rodata_address + ' -o ' + output_file + '_other' + ' ' + temp.name
